    ...
    with torch.no_grad():
        ...
        if emb_hidden_dim != -1:
            if hasattr(model.bert.embeddings, 'word_embeddings'):
                ...
                model.bert.embeddings.word_embeddings1 = torch.nn.Embedding(config.vocab_size, emb_hidden_dim,
                                                                            padding_idx=0)
                ...
                model.bert.embeddings.word_embeddings1.weight.data = model.bert.embeddings.word_embeddings1.weight.data[
                                                                     :, :emb_hidden_dim]
                model.bert.embeddings.word_embeddings2.weight.data = model.bert.embeddings.word_embeddings2.weight.data[
                                                                     :, :emb_hidden_dim]
...
def main():
    ...
    print('Loading BERT...')
    model = PrunTinyBertForSequenceClassification.from_pretrained(args.model_path, num_labels=args.num_labels)
    ...
    model = PrunTinyBertForSequenceClassification.from_pretrained(output_dir, num_labels=args.num_labels)
    torch.save(model.state_dict(), os.path.join(output_dir, 'pytorch_model.bin'))
